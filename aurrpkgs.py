...
def check_updates(packages, total, finished, output):
    for i in range(len(packages)):
        ...
        packages[i] = {k: v for k, v in packages[i].items() if k in MANDATORY_KEYS}

        # strip '-pkgrel' part and replace possible underscores with dots
        packages[i]["Version"] = re.sub(r"_", ".", packages[i]["Version"].split('-', 1)[0])

        # some repositories are not supported yet
        domain = "{uri.netloc}".format(uri=urlparse(packages[i]["URL"])).lower()

        if not any(r["url"] == domain for r in SUPPORTED_REPOS):
            output.append(APColor.warn + "[WARN]" + APColor.nc +
                               " Package " +
                               APColor.data + packages[i]["Name"] + APColor.nc +
                               ": repository " +
        ...
        try:
            with urlopen(packages[i]["URL"]) as response:
                ...
                if table_match:
                    ...
                    if version_match:
                        packages[i]["RepoVer"] = version_match.group(repo["version_match_index"])

                        # make RepoVersion to conform with Arch standards
                        # https://wiki.archlinux.org/index.php/R_package_guidelines
                        packages[i]["RepoVersion"] = re.sub(r"[:-]", ".", packages[i]["RepoVer"])
                    else:
        ...
        aurver = [int(x) for x in packages[i]["Version"].split(".")]
        repover = [int(x) for x in packages[i]["RepoVersion"].split(".")]
        ...
        if aurver < repover:
            output.append(APColor.info + "[INFO]" + APColor.nc +
                               ...
                               APColor.data + packages[i]["Name"] + APColor.nc +
                               " is outdated: " +
                               APColor.old + packages[i]["Version"] + APColor.nc +
                               " (AUR) vs " +
                               APColor.new + packages[i]["RepoVer"] + APColor.nc +
                               " (" + repo["name"] + ")")
